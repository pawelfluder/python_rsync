...
## 2. Repository Path and Import Structure
...
## 7. Zakazana Konfiguracja VS Code Launch

**Absolutnie zabronione** jest dodawanie następującej konfiguracji do pliku `.vscode/launch.json`:
...
### Dlaczego ta konfiguracja jest zabroniona?

Ta konfiguracja blokuje przycisk "Run and Debug" dla aktywnego pliku `.py`. Zamiast tego, skrypty powinny być uruchamiane bezpośrednio przez kliknięcie "Run Python File" (zielony trójkąt w prawym górnym rogu VSCode) lub przez terminal z aktywowanym środowiskiem wirtualnym.

### Poprawny sposób uruchamiania skryptów:

1. **Przez VSCode**: Kliknij "Run Python File" (zielony trójkąt) w prawym górnym rogu edytora
2. **Przez terminal**:
   ```bash
